...
/// Lists meseros with their period totals (ventas, tickets, mesas atendidas)
/// ranked by sales. The top earner gets the leader chip and the bar fills
/// proportionally to the ranking.
class WaiterPerformanceCard extends StatelessWidget {
  const WaiterPerformanceCard({
    super.key,
  ...
  final ValueChanged<WaiterPerformance>? onItemTap;
  final int maxItems;
  ...
  Widget build(BuildContext context) {
    ...
    final shown = waiters.take(maxItems).toList();
          ...
          Row(
            children: [
              Container(
                ...
                child: Icon(Icons.badge_rounded, color: MangoThemeFactory.mango, size: dpi.icon(20)),
              ),
              ...
              if (waiters.length > shown.length)
                Container(
            ...
            Expanded(
              child: Column(
                ...
                children: [
                  ...
                  Text(
                    '${data.tablesCount} ${data.tablesCount == 1 ? 'mesa' : 'mesas'} · ${data.ticketCount} ${data.ticketCount == 1 ? 'ticket' : 'tickets'} · prom. ${MangoFormatters.currency(data.averageTicket)}',
                    style: TextStyle(fontSize: dpi.font(11), color: MangoThemeFactory.mutedText(context)),
                    maxLines: 1,
                    overflow: TextOverflow.ellipsis,
                  ),
